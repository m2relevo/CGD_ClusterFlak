...
namespace ShellShock
{
    public abstract class Weapon : MonoBehaviour
    {
        ...
        public bool playerHasFired = false;
        public float timeBetweenShots = 0f;

        //public bool Shoot(Vector2 dir, Vector2 pos, int id)
        //{
        //    if (mAmmoRemaining > 0)
        //    {
        ...
        //            firedProjectile.GetComponent<ShellShock.Projectile>().Fire(pos, (dir * mMuzzleVelocity) + spr, mDamage, id);
        //            if (!hasInfiniteAmmo)
        //            {
        //                mAmmoRemaining--;
        //            }
        ...
        void Update()
        {
            if (playerHasFired && timeBetweenShots < 1 / mFireRate)
            {
                timeBetweenShots += Time.deltaTime;
            }
            if (timeBetweenShots >= /*rateOfFire*/ 1 / mFireRate)
            {
                timeBetweenShots = 0f;
        ...
        public bool ShootNew(Vector2 dir, Vector2 pos, int id)
        {
            if (mAmmoRemaining > 0)
            {
                //Vector2 spr = new Vector2(Random.value - 0.5f, Random.value - 0.5f) * mSpread;
                playerHasFired = true;
                if (playerHasFired && timeBetweenShots == 0)
                {
                    //GameObject firedProjectile = (GameObject)Instantiate(mProjectile, new Vector2(transform.position.x, transform.position.y) + dir.normalized * 1.8f, Quaternion.identity);
                    //firedProjectile.GetComponent<ShellShock.Projectile>().Fire(pos, (dir * mMuzzleVelocity) + spr, mDamage, id);
                    if (!hasInfiniteAmmo)
