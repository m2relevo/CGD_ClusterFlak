    ...
    public class TrailComponent : MonoBehaviour
    {
        ...
        void manageTrails()
        {
            ...
            if (playerScript.GetAerialPlayer())
            {
                ...
                if (activeTrails_L[activeTrails_L.Count - 1].tileType != TileType.CON)
                {
                    createNewTrail(TileType.CON, activeTrails_L, true,aP);
                }
                if (activeTrails_R[activeTrails_R.Count - 1].tileType != TileType.CON)
                {
